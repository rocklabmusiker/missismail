    ...
    <div class="row">
        <div class="col-md-12 adminUserProfile_menu">
            <div class="panel panel-success">
                <div class="panel-heading">Заказы с нашей помощью {{ $user->name }} {{ $user->lastname }}</div>
                ...
                <div class="panel-body">
                    ...
                    <div class="adminUserProfileTable">
                    	@if(isset($user))
	                    	...
	                    	@if(isset($status))
		                    	...
		                    	<table class="table table-bordered table-hover table-responsive">     
			                        <tr>
			                            <th style="width: 30%;">Ссылка на продукт</th>
			                            <td>{{ $status->link }}</td>
			                        </tr>
			                        <tr>
	                    	...
	                    	<table class="table table-bordered table-hover table-responsive">     
		                        <tr>
		                        	...
		                        	@endif
		                        	<td>
		                        		<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpDeleteOrder', ['id' => $user->id, 'id_order' => $id_order->id]) }}">
		                        			{{ csrf_field() }}
				 							{{ method_field('DELETE') }}
		                        ...
		                        <tr>
		                        	...
		                        	<td>{{ $user->debt / 100 }} €</td>
		                        	<td>
		                        		<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpUpdateDebt', ['id' => $user->id, 'id_order' => $id_order->id]) }}">
		                        			{{ csrf_field() }}
				 							{{ method_field('PUT') }}
				 							<div class="form-group">
		                        ...
		                        <tr>
		                        	...
		                        	<td>{{ $user->money }} €</td>
		                        	<td>
		                        		<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpUpdateMoney', ['id' => $user->id, 'id_order' => $id_order->id]) }}">
		                        			{{ csrf_field() }}
				 							{{ method_field('PUT') }}
				 							<div class="form-group">
		                        ...
		                        <tr>
		                        	...
		                        	<td>
		                        		<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpAccessBuySelf', ['id' => $user->id, 'id_order' => $id_order->id ]) }}">
		                        			{{ csrf_field() }}
				 							{{ method_field('PUT') }}
				 							<div class="form-group">
		                        ...
		                        @if($status)
			                        <tr>
			                        	...
			                        	<td>
			                        		<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpUpdateStatus', ['id' => $user->id, 'id_order' => $id_order->id]) }}">
			                        			{{ csrf_field() }}
					 							{{ method_field('PUT') }}
					 							<div class="form-group">
                    ...
                    <div class="adminUserProfileTable">
                    	...
                    	<table class="table table-bordered table-hover table-responsive">
                    		<tr>
	                        	<td style="width:50%;">
	                        		<form class="form-horizontal memo_form" method="post" action="{{ route('adminUserProfilOrderWithHelpMemo', ['id' => $user->id, 'id_order' => $id_order->id]) }}">
	                        			{{ csrf_field() }}
			 							{{ method_field('PUT') }}
			 							<div class="form-group">
	                        	...
	                        	<td>
	                        		@if(isset($memo))
		                        		<p>{{ $memo->text }}</p>	
	                        		@endif
	                        		<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpDeleteMemo', ['id' => $user->id, 'id_order' => $id_order->id]) }}">
	                        			{{ csrf_field() }}
			 							{{ method_field('DELETE') }}
										<div class="form-group">
                    ...
                    @if(isset($user))
	                    <div class="adminUserProfileTable"> 
							...
							<form action="{{ route('adminUserProfilOrderWithHelpEmail', ['id' => $user->id, 'id_order' => $id_order->id]) }}" class="adminUserProfileForm" method="post" enctype="multipart/form-data">
								{{ csrf_field() }}
								<div class="form-group">
								...
								<div class="form-group">
									...
									<textarea class="form-control" name="text" id="editorHelp" rows="3"></textarea>
								</div>
								<div class="form-group">
                    	...
                    	<ul class="adminUserProfile_content" id="adminUserProfile_content">
							<li class="adminUserProfile_accordion">
								...
								<div class="adminUserProfile_text adminUserProfile_acc">
									<h4>Сообщение</h4>
									<p>Здравствуйте {{ $user->name}} {{ $user->lastname }}, вы сделали заказ на выкуп товара с нашей помощью.</p> 
									<p>Ваш заказ принят и отправлен на обработку. Мы свяжимся с вами в ближайщее время.</p>
							...
							<li class="adminUserProfile_accordion">
								<div class="adminUserProfile_top-botton">
									<h4>Тема сообщения</h4>
									<p>Не хватает средств для выкупа товара</p>
								</div>
								<div class="adminUserProfile_text adminUserProfile_acc">
									<h4>Сообщение</h4>
									<p>Здравствуйте {{ $user->name}} {{ $user->lastname }}, для того чтобы мы могли выкупить ваш заказ, вам необходимо пополнить ваш счёт на минимум "здесь сумма".</p>
									<p> До получения необходимой суммы, работа с вашим заказом будет приостановлена.</p>
									<p>С уважением, команда MissisMail.</p>
									<p>ps. Если вы не знаете, как пополнить личный счёт, пройдите в раздел "Инструкции" и просмотрите видеоурок или обратитесь в Тех.поддержку.</p>
								</div>
							</li>
							<li class="adminUserProfile_accordion">
								<div class="adminUserProfile_top-botton">
									<h4>Тема сообщения</h4>
									<p>Баланс пополнен, выкуп товара</p>
								</div>
								<div class="adminUserProfile_text adminUserProfile_acc">
									<h4>Сообщение</h4>
									<p>Здравствуйте {{ $user->name}} {{ $user->lastname }}, ваш баланс пополнен на сумму "здесь сумма".</p>
									<p> Товар выкуплен и готовится к отправке на наш склад в Германии.</p>
									<p>Мы сообщим вам о поступление товара на наш склад. Доставка длиться от 3 до 5 дней.</p>
									<p>С уважением, команда MissisMail.</p>
								</div>
							</li>
							<li class="adminUserProfile_accordion">
								...
								<div class="adminUserProfile_text adminUserProfile_acc">
									...
									<p>После оплаты счёта, посылка будет передана на почтовое отделение, для последующей доставки её на ваш адрес.</p>
									<p>С уважением, команда MissisMail.</p>
								</div>
							</li>	
						</ul>
	                    			...
	                    			@foreach($adminMails as $adminMail)
	                					<li class="adminUserMailBox">
											<div class="adminMails_top-botton">
												...
												<a href="{{ route('adminUserProfilOrderWithHelpShowAdminMail', ['id' => $user->id, 'id_order' => $id_order->id, 'id_mail' => $adminMail->id]) }}">
	                    			...
	                    			@foreach($userMails as $userMail)
	                					<li class="userUserMailBox">
											<div class="userMails_top-botton">
												...
												<a href="{{ route('adminUserProfilOrderWithHelpShowUserMail', ['id' => $user->id, 'id_order' => $id_order->id, 'id_mail' => $userMail->id]) }}">
