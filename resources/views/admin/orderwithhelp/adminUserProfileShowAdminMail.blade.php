            ...
            <div class="panel panel-success">
                ...
                <div class="panel-body">
                    ...
                    <div class="adminUserProfileTable">
                    	@if(isset($adminMail))
                    	<h5>Дата: {{ date('d M. Y', strtotime($adminMail->created_at)) }}</h5>
                    	<h4>{{ $adminMail->theme }}</h4>
                    	<p>{{ $adminMail->text }}</p>
                    	@endif
                    </div>
                    <div class="adminUserProfileTable">
                    	<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderWithHelpDeleteAdminMail', ['id' => $user->id, 'id_order' => $id_order->id, 'id_mail' => $adminMail->id]) }}">
	            			{{ csrf_field() }}
							{{ method_field('DELETE') }}
