            ...
            <div class="panel panel-success">
                ...
                <div class="panel-body">
                    ...
                    <div class="adminUserProfileTable">
                    	@if(isset($userMail))
                    	<h5>Дата: {{ date('d M. Y H:i', strtotime($userMail->created_at)) }}</h5>
                    	<h4>{{ $userMail->theme }}</h4>
                    	<p>{{ $userMail->text }}</p>
                    	@endif
                    </div>
                    <div class="adminUserProfileTable">
                    	<form class="form-inline" method="post" action="{{ route('adminUserProfilOrderSelfDeleteUserMail', ['id' => $user->id, 'id_order' => $id_order->id, 'id_mail' => $userMail->id]) }}">
	            			{{ csrf_field() }}
							{{ method_field('DELETE') }}
